...
sequenceDiagram
    ...
    participant UI as UI 响应

    User->>Main: 点击
    Note over Main: 大组件渲染（200ms）
    User->>Main: 再次点击
    Note over Main: 等待中...
    Main->>UI: 响应（延迟）
```
...
flowchart LR
    subgraph EventLoop["事件循环"]
        Micro["微任务"] --> PreRender["渲染前"]
        PreRender --> rAF["requestAnimationFrame"]
        rAF --> Render["渲染"]
        Render --> Macro["宏任务"]
    end

    subgraph 方案对比
        ST["setTimeout(fn, 0)<br/>至少延迟 4ms"]
        PT["Promise.then()<br/>微任务，无法让出主线程"]
        MC["MessageChannel<br/>宏任务，无延迟"]
    end
```
...
```mermaid
sequenceDiagram
    participant A as 低优先级任务 A
    participant S as Scheduler
    participant B as 高优先级任务 B

    Note over A: 任务 A 开始
    A->>S: 执行 3ms
    Note over S: 高优先级任务 B 到来
    S->>S: shouldYield = true
    S->>B: 暂停 A，执行 B
    Note over B: B 执行完成
    S->>A: 恢复 A 继续执行
    Note over A: A 完成
